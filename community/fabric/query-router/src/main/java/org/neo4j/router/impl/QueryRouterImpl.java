...
public class QueryRouterImpl implements QueryRouter {
  ...
  public RouterTransactionContext beginTransaction(TransactionInfo incomingTransactionInfo) {
    var transactionBookmarkManager =
        new TransactionBookmarkManagerImpl(
            BookmarkFormat.parse(incomingTransactionInfo.bookmarks()));
    ...
    var routingInfo =
        new RoutingInfo(
            ...
            transactionInfo.routingContext(),
    ...
    try {
      var dummyTransactionMode =
          transactionInfo.accessMode().equals(AccessMode.READ)
  ...
  private void authorize(DatabaseReference sessionDb, LoginContext loginContext) {
    var databaseNameToAuthorizeFor =
        sessionDb instanceof DatabaseReferenceImpl.Internal
  ...
  public QueryExecution executeQuery(
      RouterTransactionContext context, Query query, QuerySubscriber subscriber) {
    TransactionInfo transactionInfo = context.transactionInfo();
    var statementLifecycle =
        statementLifecycles.create(
            transactionInfo.statementLifecycleTransactionInfo(),
            query.text(),
            query.parameters(),
            null);
    ...
    try {
      ...
      CypherExecutionMode executionMode =
          executionMode(queryOptions, transactionInfo.isComposite());
      ...
      var databaseTransaction =
          context.transactionFor(
              location,
              TransactionMode.from(
                  accessMode, executionMode, statementType.isReadQuery(), target.isComposite()));
  ...
  private CancellationChecker cancellationChecker(RouterTransaction routerTransaction) {
    return () ->
        routerTransaction.throwIfTerminatedOrClosed(
            () -> "Trying to process query in a closed transaction");
  }
