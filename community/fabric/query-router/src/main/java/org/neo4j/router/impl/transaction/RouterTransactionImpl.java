...
public class RouterTransactionImpl
    implements CompoundTransaction<DatabaseTransaction>, RouterTransaction {
  ...
  public DatabaseTransaction transactionFor(
    ...
    var tx =
        databaseTransactions.computeIfAbsent(
            location.databaseReference().id(),
            ref ->
                registerNewChildTransaction(
  ...
  @Override
  public void setConstituentTransactionFactory(
  ...
  private DatabaseTransaction createTransactionFor(
      Location location, LocationService locationService) {
    if (location instanceof Location.Local local) {
      var transaction =
          localDatabaseTransactionFactory.beginTransaction(
              local,
              transactionInfo,
      ...
      if (location.databaseReference() instanceof DatabaseReferenceImpl.Internal.SPD) {
        localDatabaseTransactionFactory.addSpdInformationToTransaction(
            ((LocalDatabaseTransaction) transaction).internalTransaction(),
            locationService,
            location);
      }
      return transaction;
    ...
    if (statementType == null) {
      ...
      if (!oldType.equals(type)) {
        ...
        if (allowedCombination) {
          ...
          var writeQueryAfterReadQuery =
              queryAfterQuery && !type.isReadQuery() && oldType.isReadQuery();
  ...
  public void registerAutocommitQuery(AutocommitQuery autocommitQuery) {
    throw new IllegalStateException(
        "Autocommit queries are not supported by Query Router transaction");
  }
  ...
  public void throwIfTerminatedOrClosed(Supplier<String> closedExceptionMessage) {
    ...
    if (state.get() == State.CLOSED) {
      throw new QueryRouterException(
          Status.Statement.ExecutionFailed, closedExceptionMessage.get());
    }
  }
