...
class RelationshipCreatorTest {
  ...
  void selfRelWithNoPriorRels() {
    ...
    assertThat(changeset)
        .containsChanges(node(0, nextRel(0)), rel(0, from(0), to(0), sCount(1), tCount(1)));
  }
  ...
  private void createRelationshipBetween(long fromNode, long toNode) {
    ...
    FlatRelationshipModifications data =
        new FlatRelationshipModifications(
            new FlatRelationshipModifications.RelationshipData(
                nextRelId(givenState), 0, fromNode, toNode));
