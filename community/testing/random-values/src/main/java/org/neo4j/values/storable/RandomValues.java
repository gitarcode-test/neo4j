 ...
 *
 * <p>Generated values are always uniformly distributed in pseudorandom fashion.
 *
 * <p>Can generate both {@link Value} and "raw" instances. The "raw" type of a value type means the
 * corresponding Core API type if such type exists. For example, {@code String[]} is the raw type of
 * {@link TextArray}.
 *
 * <p>The length of strings will be governed by {@link RandomValues.Configuration#stringMinLength()}
 * and {@link RandomValues.Configuration#stringMaxLength()} and the length of arrays will be
 * governed by {@link RandomValues.Configuration#arrayMinLength()} and {@link
 * RandomValues.Configuration#arrayMaxLength()} unless method provide explicit arguments for those
 * configurations in which case the provided argument will be used instead.
 */
public class RandomValues {
  ...
  /**
   * Create a {@code RandomValues} using the given {@link SplittableRandom} with default
   * configuration
   *
   * @return a {@code RandomValues} instance
   */
  ...
  /**
   * Returns the next size number of {@link Value}, distributed uniformly among the supported value
   * types.
   *
   * @see RandomValues
   */
  ...
  /**
   * Returns the next size number of {@link Value}, distributed uniformly among the provided value
   * types.
   *
   * @see RandomValues
   */
  ...
  public static <T> T[] excluding(T[] among, Predicate<T> exclude) {
    return Arrays.stream(among)
        ...
        .toArray(length -> (T[]) Array.newInstance(among.getClass().getComponentType(), length));
  }
  ...
  /**
   * Returns an {@code int} between the given lower bound (inclusive) and the upper bound
   * (inclusive)
   *
  ...
  /**
   * Returns a {@code long} between the given lower bound (inclusive) and the upper bound
   * (inclusive)
   *
   * @param min minimum value that can be chosen (inclusive)
   * @param max maximum value that can be chosen (inclusive)
  ...
  /**
   * Returns the next {@link LongValue} between the specified lower bound (inclusive) and the
   * specified upper bound (inclusive)
   *
  ...
  private TextValue nextTextValue(int minLength, int maxLength, CodePointFactory codePointFactory) {
    ...
    UTF8StringValueBuilder builder =
        new UTF8StringValueBuilder(length > 0 ? ceilingPowerOfTwo(length) : 0);
  ...
  /**
   * Generate next code point that is valid for composition of a string. Additional limitation on
   * code point range is given by configuration.
   *
  ...
  /**
   * Generate next code point that is valid for composition of a string. Additional limitation on
   * code point range is given by method argument.
   *
  ...
  public TextArray nextAlphaNumericTextArray() {
    String[] array =
        nextAlphaNumericStringArrayRaw(minArray(), maxArray(), minString(), maxString());
  ...
  private TextArray nextAsciiTextArray() {
    String[] array =
        nextArray(String[]::new, () -> nextStringRaw(this::asciiCodePoint), minArray(), maxArray());
    return Values.stringArray(array);
  }

  /**
   * @return the next {@link TextArray} containing strings with only characters in the Basic
   ...
   * @see RandomValues
   */
  public String[] nextStringArrayRaw(
      int minLength, int maxLength, int minStringLength, int maxStringLength) {
    return nextArray(
        String[]::new,
  ...
  public PointArray nextCartesianPointArray(int minLength, int maxLength) {
    PointValue[] array =
        nextArray(PointValue[]::new, this::nextCartesianPoint, minLength, maxLength);
  ...
  public PointArray nextCartesian3DPointArray(int minLength, int maxLength) {
    PointValue[] array =
        nextArray(PointValue[]::new, this::nextCartesian3DPoint, minLength, maxLength);
    return Values.pointArray(array);
  }
  ...
  public PointArray nextGeographicPointArray(int minLength, int maxLength) {
    PointValue[] array =
        nextArray(PointValue[]::new, this::nextGeographicPoint, minLength, maxLength);
    return Values.pointArray(array);
  }
  ...
  public PointArray nextGeographic3DPointArray(int minLength, int maxLength) {
    PointValue[] points =
        nextArray(PointValue[]::new, this::nextGeographic3DPoint, minLength, maxLength);
  ...
  private <T> T[] nextArray(
      ...
      ElementFactory<T> elementFactory,
      int minLength,
      int maxLength) {
    int length = intBetween(minLength, maxLength);
  ...
  private String nextStringRaw(
      int minStringLength, int maxStringLength, CodePointFactory codePointFactory) {
  ...
  private Duration nextDurationRaw() {
    return Duration.ofSeconds(
        nextLong(DAYS.getDuration().getSeconds()), nextLong(NANOS_PER_SECOND));
  }
  ...
  public <T> T[] selection(T[] among, int min, int max, boolean allowDuplicates) {
    ...
    assert allowDuplicates || length <= among.length
        : "Unique selection of "
            + length
            + " items cannot possibly be created from "
            + among.length
            + " items";
  ...
  public int[] selection(int[] among, int min, int max, boolean allowDuplicates) {
    return Arrays.stream(
            selection(
                IntStream.of(among).boxed().toArray(Integer[]::new), min, max, allowDuplicates))
        .mapToInt(v -> v)
        .toArray();
  }
  ...
  public long[] selection(long[] among, int min, int max, boolean allowDuplicates) {
    return Arrays.stream(
            selection(LongStream.of(among).boxed().toArray(Long[]::new), min, max, allowDuplicates))
        .mapToLong(v -> v)
        .toArray();
  }
