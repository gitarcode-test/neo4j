...
public class FlatRelationshipModifications implements RelationshipModifications {
        ...
        (nodeId, nodeData) -> {
          visitor.accept(
              new NodeRelationshipIds() {
                ...
                public RelationshipBatch creations() {
                  return new FlatRelationshipBatch(
                      nodeData.creations.values().stream().flatMap(Collection::stream).toList());
                }

                @Override
                public RelationshipBatch deletions() {
                  return new FlatRelationshipBatch(
                      nodeData.deletions.values().stream().flatMap(Collection::stream).toList());
                }

                @Override
                public void forEachCreationSplitInterruptible(InterruptibleTypeIdsVisitor visitor) {
                  for (Map.Entry<Integer, List<RelationshipData>> entry :
                      nodeData.creations.entrySet()) {
                    if (visitor.test(
                        new FlatNodeRelationshipTypeIds(
                            entry.getKey(), entry.getValue(), nodeId))) {
                      break;
                    }
                  }
                }
  ...
  public static RelationshipModifications singleCreate(
      long id, int type, long startNode, long endNode) {
  ...
  public static RelationshipModifications singleCreate(
    ...
    return new FlatRelationshipModifications(
        relationship(id, type, startNode, endNode, properties));
  }
  ...
  public static RelationshipModifications modifications(
      RelationshipData[] creations, RelationshipData[] deletions) {
  ...
  public static RelationshipModifications singleDelete(
      long id, int type, long startNode, long endNode) {
    return new FlatRelationshipModifications(
  ...
  private static class FlatRelationshipBatch implements RelationshipBatch {
    ...
    @Override
    public <E extends Exception> void forEach(RelationshipVisitorWithProperties<E> relationship)
        throws E {
  ...
  private static class FlatNodeRelationshipTypeIds implements NodeRelationshipTypeIds {
    ...
    public RelationshipBatch out() {
      return new FlatRelationshipBatch(
          relationships.stream().filter(r -> r.direction(nodeId) == OUTGOING).toList());
    }

    @Override
    public RelationshipBatch in() {
      return new FlatRelationshipBatch(
          relationships.stream().filter(r -> r.direction(nodeId) == INCOMING).toList());
    }
