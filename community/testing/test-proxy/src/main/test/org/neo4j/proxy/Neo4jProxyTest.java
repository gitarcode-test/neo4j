...
public class Neo4jProxyTest {
  ...
  void startStoppingShouldCorrectly() throws InterruptedException {
    ...
    await()
        .atMost(awaitTime)
        .untilAsserted(() -> assertThat(echoServer1.clientConnected).isFalse());
    await()
        .atMost(awaitTime)
        .untilAsserted(() -> assertThat(echoClient1.connectToServer).isFalse());
    await().atMost(awaitTime).untilAsserted(() -> assertThat(echoServer1.messageRead).isFalse());
    await()
        .atMost(awaitTime)
        .untilAsserted(() -> assertThat(echoClient1.messageReceived).isFalse());
