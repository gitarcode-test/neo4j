...
final class ThreadPool {
  ...
  ThreadPool(
    ...
    threadFactory =
        parameters.providedThreadFactory.newSchedulerThreadFactory(group, parentThreadGroup);
  ...
  public <T> JobHandle<T> submit(JobMonitoringParams jobMonitoringParams, Callable<T> job) {
    ...
    var placeHolder =
        new RegisteredJob(
            -1, completedFuture(Void.TYPE), NOT_MONITORED, Instant.now(), new AtomicBoolean());
    registry.put(registryKey, placeHolder);
    try {
      var future = executor.submit(registeredJob);
      registry.replace(
          registryKey, new RegisteredJob(jobId, future, jobMonitoringParams, submitted, running));
