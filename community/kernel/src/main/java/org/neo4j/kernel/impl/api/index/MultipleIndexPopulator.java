 ...
 *
 * <ul>
 *   <li>A {@link StoreScan} is created through {@link #createStoreScan(CursorContextFactory)}. The
 *       store scan is started by {@link StoreScan#run(StoreScan.ExternalUpdatesCheck)}, which is a
 *       blocking call and will scan the entire store and generate updates that are fed into the
 *       {@link IndexPopulator populators}. Only a single call to this method should be made during
 *       the life time of a {@link MultipleIndexPopulator} and should be called by the same thread
 *       instantiating this instance.
 *   <li>{@link #queueConcurrentUpdate(IndexEntryUpdate)} which queues updates which will be read by
 *       the thread currently executing the store scan and incorporated into that data stream. Calls
 *       to this method may come from any number of concurrent threads.
 * </ul>
 *
 * <p>Usage of this class should be something like:
 *
 * <ol>
 *   <li>Instantiation.
 *   <li>One or more calls to {@link #addPopulator(IndexPopulator, IndexProxyStrategy,
 *       FlippableIndexProxy, FailedIndexProxyFactory)}.
 *   <li>Call to {@link #create(CursorContext)} to create data structures and files to start
 *       accepting updates.
 *   <li>Call to {@link #createStoreScan(CursorContextFactory)} and {@link
 *       StoreScan#run(StoreScan.ExternalUpdatesCheck)}(blocking call).
 *   <li>While all nodes are being indexed, calls to {@link
 *       #queueConcurrentUpdate(IndexEntryUpdate)} are accepted.
 *   <li>Call to {@link #flipAfterStoreScan(CursorContext)} after successful population, or {@link
 *       #cancel(Throwable, CursorContext)} if not
 * </ol>
 *
 * <p>It is possible for concurrent updates from transactions to arrive while index population is in
 * progress. Such updates are inserted in the {@link #queueConcurrentUpdate(IndexEntryUpdate)
 * queue}. When store scan notices that queue size has reached {@link #queueThreshold} then it
 * drains all batched updates and waits for all job scheduler tasks to complete and flushes updates
 * from the queue using {@link MultipleIndexUpdater}. If queue size never reaches {@link
 * #queueThreshold} than all queued concurrent updates are flushed after the store scan in {@link
 * #flipAfterStoreScan(CursorContext)}.
 *
 * <p>
 */
public class MultipleIndexPopulator implements StoreScan.ExternalUpdatesCheck, AutoCloseable {
  ...
  // Concurrency queue since multiple concurrent threads may enqueue updates into it. It is
  // important for this queue
  ...
  // Populators are added into this list. The same thread adding populators will later call
  // #createStoreScan.
  ...
  public MultipleIndexPopulator(
    ...
    this.queueThreshold =
        config.get(GraphDatabaseInternalSettings.index_population_queue_threshold);
    this.batchMaxByteSizeScan =
        config.get(GraphDatabaseInternalSettings.index_population_batch_max_byte_size).intValue();
  }
  ...
  IndexPopulation addPopulator(
    ...
    IndexPopulation population =
        createPopulation(populator, indexProxyStrategy, flipper, failedIndexProxyFactory);
  ...
  StoreScan createStoreScan(CursorContextFactory contextFactory) {
    ...
    if (type == EntityType.RELATIONSHIP) {
      StoreScan innerStoreScan =
          storeView.visitRelationships(
              entityTokenIds,
              propertySelection,
  ...
  /**
   * Queues an update to be fed into the index populators. These updates come from changes being
   * made to storage while a concurrent scan is happening to keep populators up to date with all
   * latest changes.
   *
  ...
  /**
   * Cancel all {@link IndexPopulation index populations}, putting the indexes in {@link
   * InternalIndexState#FAILED failed state}. To repopulate them they will need to be dropped and
   * recreated.
   *
  ...
  /**
   * Cancel a single {@link IndexPopulation index population}, putting the index in {@link
   * InternalIndexState#FAILED failed state}. To repopulate the index it needs to be dropped and
   * recreated.
   *
  ...
  protected void cancel(
      IndexPopulation population, Throwable failure, CursorContext cursorContext) {
    if (!removeFromOngoingPopulations(population)) {
      return;
    }

    // If the cause of index population failure is a conflict in a (unique) index, the conflict is
    // the failure
    ...
    log.error(
        format("Failed to populate index: [%s]", population.userDescription(tokenNameLookup)),
        failure);
  ...
  /**
   * Close this {@link MultipleIndexPopulator multiple index populator}. This means population job
   * has finished, successfully or unsuccessfully and resources can be released.
   *
   * <p>Note that {@link IndexPopulation index populations} cannot be closed. Instead, the
   * underlying {@link IndexPopulator index populator} is closed by {@link
   * #flipAfterStoreScan(CursorContext)}, {@link #cancel(IndexPopulation, Throwable, CursorContext)}
   * or {@link #stop(IndexPopulation, CursorContext)}.
   */
  @Override
  public void close() {
    phaseTracker.stop();
   ...
   *
   * <p>The last updates will be applied to every index, tell {@link IndexPopulator index
   * populators} that scan has been completed, {@link IndexStatisticsStore index statistics store}
   * will be updated with {@link IndexSample index samples}, {@link SchemaState schema cache} will
   * be cleared, {@link IndexPopulator index populators} will be closed and {@link IndexProxy index
   * proxy} will be {@link FlippableIndexProxy#flip(Callable, FailedIndexProxyFactory) flipped} to
   * {@link OnlineIndexProxy online}, given that nothing goes wrong.
   */
  ...
  /**
   * Stop all {@link IndexPopulation index populations}, closing backing {@link IndexPopulator index
   * populators}, keeping them in {@link InternalIndexState#POPULATING populating state}.
   */
  ...
  /**
   * Close specific {@link IndexPopulation index population}, closing backing {@link IndexPopulator
   * index populator}, keeping it in {@link InternalIndexState#POPULATING populating state}.
   *
  ...
  public void applyExternalUpdates(long currentlyIndexedNodeId) {
    ...
    try (MultipleIndexUpdater updater = newPopulatingUpdater(cursorContext)) {
      do {
        ...
        // Since updates can be added concurrently with us draining the queue simply setting the
        // value to 0
        // after drained will not be 100% synchronized with the queue contents and could potentially
        // cause a
        // large
        // drift over time. Therefore each update polled from the queue will subtract its size
        // instead.
        ...
        if (update != null && update.getEntityId() <= currentlyIndexedNodeId) {
          ...
          log.info(
              "Skipped %s from queue", update == null ? null : update.describe(tokenNameLookup));
        }
  ...
  private void forEachPopulation(
      ThrowingConsumer<IndexPopulation, Exception> action, CursorContext cursorContext) {
    for (IndexPopulation population : populations) {
      try {
  ...
  public String toString() {
    String updatesString =
        populations.stream().map(Object::toString).collect(joining(", ", "[", "]"));

    return "MultipleIndexPopulator{activeTasks="
        + activeTasks
        + ", "
        + "batchedUpdatesFromScan = "
        + updatesString
        + ", concurrentUpdateQueue = "
        + concurrentUpdateQueue.size()
        + "}";
  }
  ...
  public class IndexPopulation implements SchemaDescriptorSupplier {
    ...
    /**
     * Disconnect this single {@link IndexPopulation index population} from ongoing multiple index
     * population and close {@link IndexPopulator index populator}, leaving it in {@link
     * InternalIndexState#POPULATING populating state}.
     */
    ...
    /**
     * Disconnect this single {@link IndexPopulation index population} from ongoing multiple index
     * population and {@link IndexPopulator#drop() drop} the index.
     */
    ...
    private void disconnect(Runnable specificPopulatorOperation) {
      ...
      try {
        if (populationOngoing) {
          // First of all remove this population from the list of ongoing populations so that it
          // won't receive
          // more updates.
          // This is good because closing the populator may wait for an opportunity to perform the
          // close,
    ...
    private void logCompletionMessage() {
      log.info(
          "Index creation finished for index [%s].", indexProxyStrategy.getIndexUserDescription());
    }
      ...
      IndexPopulator.PopulationWorkScheduler populationWorkScheduler =
          new IndexPopulator.PopulationWorkScheduler() {
            ...
            public <T> JobHandle<T> schedule(
              ...
              var description =
                  descriptionSupplier.getJobDescription(
  ...
  private class PropertyScanConsumerImpl implements PropertyScanConsumer {
    ...
    private void addFromScan(List<EntityUpdates> entityUpdates, CursorContext cursorContext) {
      // This is called from a full store node scan, meaning that all node properties are included
      // in the
      // EntityUpdates object. Therefore no additional properties need to be loaded.
      Map<IndexPopulation, List<IndexEntryUpdate<IndexPopulation>>> updates =
          new HashMap<>(populations.size());
      for (EntityUpdates update : entityUpdates) {
        for (IndexEntryUpdate<IndexPopulation> indexUpdate :
            update.valueUpdatesForIndexKeys(populations)) {
      ...
      for (Map.Entry<IndexPopulation, List<IndexEntryUpdate<IndexPopulation>>> entry :
          updates.entrySet()) {
        try {
  ...
  /**
   * A delegating {@link StoreScan} with the only functionality being logging when the scan is
   * completed.
   */
