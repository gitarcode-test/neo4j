 ...
 *
 * <p>This class maintains references to all transactions, a pool of passive kernel transactions,
 * and provides capabilities for enumerating all running transactions. During normal operation,
 * acquiring new transactions and enumerating live ones requires no synchronization (although the
...
public class KernelTransactions extends LifecycleAdapter
   ...
   *
   * <p>This data structure is *only* updated when brand-new transactions are created, or when
   * transactions are disposed of. During normal operation (where all transactions come from and are
   * returned to the pool), this will be left in peace, working solely as a collection of references
   * to all transaction objects (idle and active) in the database.
   *
   * <p>As such, it provides a good mechanism for listing all transactions without requiring
   * synchronization when starting and committing transactions.
   */
  ...
  /**
   * Kernel transactions component status. True when stopped, false when started. Will not allow to
   * start new transaction by stopped instance of kernel transactions. Should simplify tracking of
   * stopped component usage by up the stack components.
   */
  ...
  public KernelTransactions(
    ...
    this.txPool =
        new MonitoredTransactionPool(
            new GlobalKernelTransactionPool(
                allTransactions,
                new KernelTransactionImplementationFactory(allTransactions, tracers)),
            activeTransactionCounter,
            config);
    this.enrichmentStrategy =
        this.databaseDependencies.resolveDependency(ApplyEnrichmentStrategy.class);
    this.securityLog = this.databaseDependencies.resolveDependency(AbstractSecurityLog.class);
    this.databaseSerialGuard =
        multiVersioned ? new MultiVersionDatabaseSerialGuard(allTransactions) : EMPTY_GUARD;
  ...
  /**
   * Give an approximate set of all transactions currently running. This is not guaranteed to be
   * exact, as transactions may stop and start while this set is gathered.
   *
  ...
  /**
   * Give an approximate set of all transactions currently executing. In contrast to {@link
   * #activeTransactions}, this also includes transactions in the closing state, e.g. committing or
   * rolling back. This is not guaranteed to be exact, as transactions may stop and start while this
   * set is gathered.
   *
  ...
  public void init() throws Exception {
    ...
    config.addListener(
        ...
        (before, after) -> transactionMemoryPool.setSize(after));
  }
  ...
  /**
   * Do not allow new transactions to start until {@link #unblockNewTransactions()} is called.
  ...
  /**
   * Allow new transactions to be started again if current thread is the one who called {@link
   * #blockNewTransactions()}.
   *
   * @throws IllegalStateException if current thread is not the one that called {@link
   *     #blockNewTransactions()}.
   */
   ...
   *
   * <p><b>Note:</b> this method is package-private for testing <b>only</b>.
   *
  ...
  private static class GlobalKernelTransactionPool
    ...
    GlobalKernelTransactionPool(
        Set<KernelTransactionImplementation> transactions,
  ...
  static class MonitoredTransactionPool implements Pool<KernelTransactionImplementation> {
    ...
    MonitoredTransactionPool(
        GlobalKernelTransactionPool delegate,
        AtomicInteger activeTransactionCounter,
        Config config) {
    ...
    private void verifyTransactionsLimit() {
      ...
      } while (!activeTransactionCounter.weakCompareAndSetAcquire(
