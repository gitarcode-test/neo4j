...
public abstract class DataLookup {
  ...
  public Node getNodeById(long id) {
    if (id < 0 || !dataRead().nodeExists(id)) {
      throw new NotFoundException(
          ...
          new EntityNotFoundException(EntityType.NODE, valueOf(id)));
    }
  ...
  public Node getNodeByElementId(String elementId) {
    ...
    if (!dataRead().nodeExists(nodeId)) {
      throw new NotFoundException(
          ...
          new EntityNotFoundException(EntityType.NODE, elementId));
    }
  ...
  public ResourceIterator<Node> findNodes(
      Label myLabel, String key, String value, StringSearchMode searchMode) {
    checkLabel(myLabel);
    ...
    IndexDescriptor index =
        findUsableMatchingIndex(
            SchemaDescriptors.forLabel(labelId, propertyId), IndexType.TEXT, query);
    ...
    if (index == IndexDescriptor.NO_INDEX
      ...
      index =
          findUsableMatchingIndex(SchemaDescriptors.forLabel(labelId, propertyId), allStringQuery);
  ...
  public ResourceIterator<Node> findNodes(Label myLabel, String key, Object value) {
    ...
    PropertyIndexQuery.ExactPredicate query =
        PropertyIndexQuery.exact(propertyId, Values.of(value, false));
    IndexDescriptor index =
        findUsableMatchingIndex(SchemaDescriptors.forLabel(labelId, propertyId), query);
    return nodesByLabelAndProperty(labelId, query, index);
  }

  public ResourceIterator<Node> findNodes(
      Label label, String key1, Object value1, String key2, Object value2) {
    checkLabel(label);
  ...
  public ResourceIterator<Relationship> findRelationships(
    ...
    IndexDescriptor index =
        findUsableMatchingIndex(
            SchemaDescriptors.forRelType(typeId, propertyId), IndexType.TEXT, query);
    ...
    if (index == IndexDescriptor.NO_INDEX
      ...
      PropertyIndexQuery.RangePredicate<?> allStringQuery =
          PropertyIndexQuery.range(propertyId, (String) null, false, null, false);
      index =
          findUsableMatchingIndex(SchemaDescriptors.forRelType(typeId, propertyId), allStringQuery);
      if (index != IndexDescriptor.NO_INDEX && index.getCapability().supportsReturningValues()) {
  ...
  public ResourceIterator<Relationship> findRelationships(
    ...
    PropertyIndexQuery.ExactPredicate query =
        PropertyIndexQuery.exact(propertyId, Values.of(value, false));
    IndexDescriptor index =
        findUsableMatchingIndex(SchemaDescriptors.forRelType(typeId, propertyId), query);
    return relationshipsByTypeAndProperty(typeId, query, index);
  }
  ...
  private ResourceIterator<Relationship> allRelationshipsByTypeWithoutIndex(int typeId) {
    ...
    var filteredCursor =
        new FilteringRelationshipScanCursorWrapper(cursor, CursorPredicates.hasType(typeId));
    return new TrackedCursorIterator<>(
        filteredCursor,
  ...
  private IndexDescriptor findUsableMatchingIndex(
      SchemaDescriptor schemaDescriptor, IndexQuery... query) {
    return firstOrDefault(
        getMatchingOnlineIndexes(schemaDescriptor, query), IndexDescriptor.NO_INDEX);
  }

  private Iterator<IndexDescriptor> getMatchingOnlineIndexes(
      SchemaDescriptor schemaDescriptor, IndexQuery... query) {
    SchemaRead schemaRead = schemaRead();
    Iterator<IndexDescriptor> iterator = schemaRead.index(schemaDescriptor);
    return filter(
        index -> indexIsOnline(schemaRead, index) && indexSupportQuery(index, query), iterator);
  }
  ...
  private ResourceIterator<Node> nodesByLabelAndPropertyWithFiltering(
    ...
    try {
      NodeValueIndexCursor cursor =
          cursors().allocateNodeValueIndexCursor(cursorContext(), memoryTracker());
  ...
  private ResourceIterator<Node> getNodesByLabelAndPropertyWithoutPropertyIndex(
    ...
    var index =
        findUsableMatchingIndex(SchemaDescriptors.ANY_TOKEN_NODE_SCHEMA_DESCRIPTOR, tokenQuery);
  ...
  private TrackedCursorIterator<FilteringNodeCursorWrapper, Node>
      getNodesByLabelAndPropertyViaAllNodesScan(int labelId, PropertyIndexQuery[] queries) {
    var nodeCursor = cursors().allocateNodeCursor(cursorContext(), memoryTracker());
    var labelFilteredCursor =
        new FilteringNodeCursorWrapper(nodeCursor, CursorPredicates.hasLabel(labelId));

    var propertyCursor = cursors().allocatePropertyCursor(cursorContext(), memoryTracker());
    var propertyFilteredCursor =
        new FilteringNodeCursorWrapper(
            labelFilteredCursor,
            nodeMatchProperties(queries, propertyCursor),
  ...
  private ResourceIterator<Node> nodesByLabelAndProperties(
    ...
    IndexDescriptor index =
        findUsableMatchingCompositeIndex(
    ...
    if (index != IndexDescriptor.NO_INDEX) {
      try {
        ...
        var cursor =
            cursors().allocateRelationshipTypeIndexCursor(cursorContext(), memoryTracker());
        dataRead()
            .relationshipTypeScan(session, cursor, unconstrained(), tokenQuery, cursorContext());
  ...
  private ResourceIterator<Relationship> getRelationshipsByTypeAndPropertyViaAllRelsScan(
    ...
    var typeFiltered =
        new FilteringRelationshipScanCursorWrapper(
            relationshipScanCursor, CursorPredicates.hasType(typeId));

    var propertyCursor = cursors().allocatePropertyCursor(cursorContext(), memoryTracker());
    var propertyFilteredCursor =
        new FilteringRelationshipScanCursorWrapper(
            typeFiltered,
            relationshipMatchProperties(queries, propertyCursor),
            List.of(propertyCursor));
  ...
  private static boolean indexSupportQuery(IndexDescriptor index, IndexQuery[] query) {
    return stream(query)
        .allMatch(q -> index.getCapability().isQuerySupported(q.type(), q.valueCategory()));
  }
  ...
  private static PropertyIndexQuery getIndexQuery(
      String value, StringSearchMode searchMode, int propertyId) {
    return switch (searchMode) {
      ...
      case CONTAINS ->
          PropertyIndexQuery.stringContains(propertyId, utf8Value(value.getBytes(UTF_8)));
    };
  }
  ...
  private static PropertyIndexQuery.ExactPredicate[] convertToQueries(
    ...
    PropertyIndexQuery.ExactPredicate[] queries =
        new PropertyIndexQuery.ExactPredicate[propertyValues.size()];
    int i = 0;
    for (Map.Entry<String, Object> entry : propertyValues.entrySet()) {
      queries[i++] =
          PropertyIndexQuery.exact(
              tokenRead.propertyKey(entry.getKey()), Values.of(entry.getValue(), false));
    }
    return queries;
  }
    ...
    for (int i = 1; i < propertyIds.length; i++) {
      ...
      if (curr == prev) {
        throw new IllegalArgumentException(
            format(
                "Provided two queries for property %s. Only one query per property key can be"
                    + " performed",
  ...
  private static PropertyIndexQuery[] getReorderedIndexQueries(
      int[] indexPropertyIds, PropertyIndexQuery[] queries) {
  ...
  protected abstract Relationship newRelationshipEntity(
