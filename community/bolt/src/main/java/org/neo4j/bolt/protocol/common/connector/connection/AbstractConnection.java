...
public abstract class AbstractConnection implements ConnectionHandle {
  ...
  protected final AtomicReference<StructRegistry<Connection, Value>> structRegistry =
      new AtomicReference<>();
  ...
  @Override
  public void notifyListenersSafely(
      String eventName, Consumer<ConnectionListener> notifierFunction) {
  ...
  public void selectProtocol(BoltProtocol protocol) {
    ...
    if (!this.protocol.compareAndSet(null, protocol)) {
      throw new IllegalStateException(
          "Protocol has already been selected for connection " + this.id);
    }
    ...
    // update the writer pipeline to include Bolt protocol specific writer implementations for
    ...
    // also enable any implicitly enabled features within the protocol version as we do not want
    // these to be enabled
    // again if negotiated through one of the later mechanisms
    this.features.set(Collections.unmodifiableSet(protocol.features()));

    // allocate a new state machine for the desired protocol version to prepare the connection for
    // handling requests
  ...
  public AuthenticationFlag logon(Map<String, Object> token) throws AuthenticationException {
    this.connectionInfo =
        new BoltConnectionInfo(
            this.id, userAgent, this.clientAddress(), this.serverAddress(), this.boltAgent);
  ...
  public void impersonate(String userToImpersonate) throws AuthenticationException {
    ...
    this.impersonationContext =
        this.connector.authentication().impersonate(loginContext, userToImpersonate);
  ...
  public void clearImpersonation() {
    // ignore the call entirely if there is no impersonation currently present on this connection to
    // avoid any
  ...
  public void resolveDefaultDatabase() {
    ...
    if (loginContext == null) {
      throw new IllegalStateException(
          "Cannot resolve default database: Connection has not been authenticated");
    }
