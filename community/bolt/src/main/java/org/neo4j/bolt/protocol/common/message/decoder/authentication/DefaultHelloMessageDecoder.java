...
public class DefaultHelloMessageDecoder implements MessageDecoder<HelloMessage> {
  ...
  protected static final String FILED_NOTIFICATIONS_DISABLED_CATEGORIES =
  ...
  public HelloMessage read(Connection ctx, PackstreamBuf buffer, StructHeader header)
    ...
    var meta =
        AuthenticationMetadataUtils.convertExtraMap(reader, buffer.getTarget().readableBytes());
    ...
    return new HelloMessage(
        userAgent, features, routingContext, authToken, notificationsConfig, boltAgent);
  }
  ...
  protected List<Feature> readFeatures(Map<String, Object> meta) {
    ...
    // since this is an optional protocol feature which was introduced after the original spec was
    // written, we're
    // not going to strictly validate the list or its contents
    return Collections.emptyList();
  }

  protected RoutingContext readRoutingContext(Map<String, Object> meta)
      throws PackstreamReaderException {
  ...
  protected Map<String, String> readBoltAgent(Map<String, Object> meta)
    ...
    if (!map.containsKey("product")) {
      throw new IllegalStructArgumentException(
          FIELD_BOLT_AGENT, "Expected map to contain key: 'product'.");
    }
    return map;
  }
  ...
  private static Map<String, String> convertToStringMap(Object property, String field)
    ...
    for (var entry : propertyMap.entrySet()) {
      if (!(entry.getKey() instanceof String key && entry.getValue() instanceof String value)) {
        throw new IllegalStructArgumentException(
            field, "Must be a map with string keys and string values.");
      }
