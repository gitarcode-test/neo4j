...
class AtomicSchedulingConnectionTest {
  ...
  void prepareConnection() {
    ...
    Mockito.doReturn(DEFAULT_DB)
        .when(this.defaultDatabaseResolver)
        .defaultDatabase(ArgumentMatchers.any());
    ...
    this.connection =
        new AtomicSchedulingConnection(
            this.connector,
            CONNECTION_ID,
            this.channel,
            CONNECTION_TIME,
  ...
  void shouldExecuteJobs() throws BrokenBarrierException, InterruptedException {
    ...
    // newly created connections should consider themselves to be idle as there are no queued jobs
    // nor have they
    ...
    // however, once a job is submitted, they should consider themselves to be busy while their
    ...
    // this should still be true if the job begins executing (it will no longer be considered to
    // have pending jobs,
    // however)
  ...
  void shouldInterrupt() {
    ...
    // interrupts can stack, if interrupted twice the connection should still identify itself as
    // interrupted
    this.connection.interrupt();
    ...
    // if reset another time, the second interrupt should clear and the connection should return to
    // its normal state
    Assertions.assertThat(this.connection.reset()).isTrue();

    ConnectionHandleAssertions.assertThat(this.connection).isNotInterrupted();

    // if we interrupt it again, it should return to its interrupted state and identify itself as
    // such
    this.connection.interrupt();

    ConnectionHandleAssertions.assertThat(this.connection).isInterrupted();

    // since there is only a single active interrupt, it should return to its normal state
  ...
  void shouldIdentifyClosingState() {
    ...
    ConnectionHandleAssertions.assertThat(this.connection).isActive().isNotClosing().isNotClosed();

    this.connection.close();

    // when the connection is still busy (e.g. is currently executing or has pending jobs), the
    // connection should
    // simply be marked for closure but never actually close unless its jobs are executed
    ConnectionHandleAssertions.assertThat(this.connection).isNotActive().isClosing().isNotClosed();
  }
  ...
  void shouldImmediatelyCloseWhenInIdle() throws Exception {
    ...
    // when closing in idle, the connection should immediately transition to closed as to not clog
    // up the worker
    // pool without good reason
    ConnectionHandleAssertions.assertThat(this.connection).isNotActive().isNotClosing().isClosed();
    ...
    // the close future should also be marked as completed as a result of this call so that
    // dependent
  ...
  void shouldCloseFromWorkerThreadWhenScheduled() throws StateMachineException {
    ...
    // if we now close the connection, it should be marked for closure but not immediately close as
    // it still has
    // pending jobs
    this.connection.close();
    ...
    // if we invoke the submitted runnable, the connection should process the remaining jobs and
    // finally close
    ...
    // execute - it is the callers responsibility to not kill connections which remain active if
    // possible
  ...
  void shouldNotifyListeners() {
    ...
    // listeners should be notified of the addition through their onListenerAdded function in order
    // to initialize
    ...
    // once removed, listeners should no longer receive notifications through either of the provided
    // APIs
    this.connection.notifyListeners(
        connectionListener -> connectionListener.onConnectionClosed(true));
  ...
  void shouldNotThrowWhenSafelyNotifyingListeners() {
    ...
    // notifyListenersSafely should invoke all listeners in order even when an exception is thrown
    // (the resulting
    // exception does not bubble up and will be logged instead) - this functionality is primarily
    // provided for the
  ...
  void shouldAuthenticate() throws AuthenticationException {
    ...
    // there should be no authentication flags set as the credentials were deemed valid at the time
    // of
    // authentication
    Assertions.assertThat(flags).isNull();
    Mockito.verify(result, Mockito.times(2))
        .credentialsExpired(); // called two times due to logging

    // the client connection info should be updated to reflect the authenticated user within
    // administrative
    // procedures such as list connection
    var clientInfoCaptor = ArgumentCaptor.forClass(ClientConnectionInfo.class);
    Mockito.verify(this.authentication)
        .authenticate(ArgumentMatchers.eq(token), clientInfoCaptor.capture());
  ...
  void shouldImpersonateUser() throws AuthenticationException {
    ...
    Mockito.doReturn(IMPERSONATED_DB)
        .when(this.defaultDatabaseResolver)
        .defaultDatabase(IMPERSONATED_USER);
    ...
    // the indicated loginContext is replaced with the impersonated context for the duration of the
    // impersonation so
    // that all executions scheduled through this connection make use of this authentication instead
    // of the original
    Assertions.assertThat(this.connection.loginContext())
        .isNotSameAs(this.loginContext)
        .isSameAs(impersonationContext);

    // Connection username should remain on its original value as this is the value we report within
    // administrative
    ...
    // the default database should be resolved again in order to switch to the impersonated user's
    // home database if
    // any has been set
    ...
    // also make sure that the impersonation is correctly logged to the debug log as this
    // information will be
  ...
  @Test
  void shouldFailWithNullPointerExceptionWhenImpersonatedUserIsNull()
  ...
  @Test
  void clearImpersonationShouldBeIgnoredWhenNoImpersonationIsActive()
      throws AuthenticationException {
    this.authenticate();
  ...
  void shouldResolveDefaultDatabase() throws AuthenticationException {
    ...
    Mockito.doReturn(IMPERSONATED_DB)
        .when(this.defaultDatabaseResolver)
        .defaultDatabase(AUTHENTICATED_USER);
    ...
    Mockito.verify(this.defaultDatabaseResolver, Mockito.times(2))
        .defaultDatabase(AUTHENTICATED_USER);
    Mockito.verifyNoMoreInteractions(this.defaultDatabaseResolver);
