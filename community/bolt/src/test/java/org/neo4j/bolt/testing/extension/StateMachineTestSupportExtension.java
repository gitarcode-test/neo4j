...
public class StateMachineTestSupportExtension implements TestTemplateInvocationContextProvider {
  ...
  public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(
    ...
    var annotation =
        AnnotationUtil.findAnnotation(extensionContext, StateMachineTestExtension.class)
            .orElse(null);
        ...
        .map(
            fsmProvider -> {
              ...
              try {
                try {
                  dependencyProvider =
                      dependencyProviderClass
                          .getDeclaredConstructor(ExtensionContext.class)
                          .newInstance(extensionContext);
                } catch (NoSuchMethodException ignore) {
                  dependencyProvider =
                      dependencyProviderClass.getDeclaredConstructor().newInstance();
                }
  ...
  private Stream<StateMachineProvider> matchingProviders(StateMachineTest annotation) {
    ...
    if (annotation.include().length != 0) {
      var inclusions = Arrays.stream(annotation.include()).map(this::convertVersion).toList();

      includedVersions =
          includedVersions.filter(
              provider ->
                  inclusions.stream().anyMatch(included -> included.matches(provider.version())));
    }

    if (annotation.exclude().length != 0) {
      var exclusions = Arrays.stream(annotation.exclude()).map(this::convertVersion).toList();

      includedVersions =
          includedVersions.filter(
              provider ->
                  exclusions.stream().noneMatch(excluded -> excluded.matches(provider.version())));
    }
