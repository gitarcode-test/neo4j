...
public class CapabilitiesService extends LifecycleAdapter implements CapabilitiesRegistry {
  ...
  CapabilitiesService(
    ...
    this.capabilities =
        getDeclaredCapabilities(Objects.requireNonNull(declarationClasses)).entrySet().stream()
            .collect(
                Collectors.toMap(Map.Entry::getKey, e -> new CapabilityInstance<>(e.getValue())));
  ...
  void processProviders() {
    ...
    dependencies.register(
        Configuration.class, () -> resolver.resolveDependency(Configuration.class));
    dependencies.register(
        InternalLog.class,
        () -> resolver.resolveDependency(LogService.class).getUserLog(Capabilities.class));
    dependencies.register(
        DatabaseManagementService.class,
        () -> resolver.resolveDependency(DatabaseManagementService.class));

    capabilityProviders.forEach(
        p ->
            p.register(
                new CapabilityProviderContext(dependencies),
                new NamespaceAwareCapabilityRegistry(p.namespace())));
  }
  ...
  // A pure Capabilities implementation without allowing the caller to cast it into a
    ...
    public <T> void set(Capability<T> capability, T value) {
      if (!capability.name().isIn(namespace)) {
        throw new IllegalArgumentException(
            String.format(
                "provided capability %s is not in declared namespace %s",
  ...
  public static CapabilitiesService newCapabilities(
      Configuration config, DependencyResolver resolver) {
    Collection<Class<? extends CapabilityDeclaration>> declarationClasses =
                  ...
                  capabilities.merge(
                      ...
                      (oldValue, newValue) -> {
                        throw new UnsupportedOperationException(
                            format("duplicate capability %s", name));
                      }));
    }
