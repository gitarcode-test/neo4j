...
abstract class BootloaderOsAbstraction {
  ...
  protected static final ExitCodeMessageMapper NEO4J_PROCESS_EXITCODE_MAPPER =
      exitCode ->
          switch (exitCode) {
                case NeoBootstrapper.WEB_SERVER_STARTUP_ERROR_CODE ->
                    "Neo4j web server failed to start.";
                case NeoBootstrapper.GRAPH_DATABASE_STARTUP_ERROR_CODE ->
                    "Neo4j server failed to start.";
                case NeoBootstrapper.INVALID_CONFIGURATION_ERROR_CODE ->
                    "Configuration is invalid.";
                case NeoBootstrapper.LICENSE_NOT_ACCEPTED_ERROR_CODE ->
  ...
  long admin() throws CommandFailedException {
    ...
    return bootloader
        .processManager()
        .run(arguments.withAll(bootloader.additionalArgs), new AdminProcess());
  }
    ...
    if (isNotEmpty(envJavaOptions)) {
      if (isNotEmpty(bootloader.getEnv(ENV_HEAP_SIZE))) {
        bootloader
            .environment
            .err()
            .println("WARNING! HEAP_SIZE is ignored, because JAVA_OPTS is set");
      }

      // We need to turn a list of JVM options provided as one string into a list of individual
      // options.
