...
public final class Services {
  ...
  public static <T> Collection<T> loadAll(ClassLoader classLoader, Class<T> service) {
    ...
    // in application servers, osgi and alike environments, context classloader can differ from the
    // one that loads
    // neo4j libs;
    // in such cases we need to load services from both
    if (classLoader != contextCL) {
      // services from context class loader have higher precedence, so we skip duplicates by
      // comparing class
      // names.
      loadAllSafely(service, classLoader)
          .forEach(provider -> providers.putIfAbsent(provider.getClass().getName(), provider));
    }
  ...
  public static <T, K> Optional<T> load(Class<T> service, K key, Function<T, K> keyAccessor) {
    ...
    if (matches.size() > 1) {
      throw new RuntimeException(
          format("Found multiple service providers %s[%s]: %s", service, key, matches));
    }
