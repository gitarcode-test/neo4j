...
/**
 * Utility class that manages directory structure for a partitioned lucene index. It is aware of the
 * {@link FileSystemAbstraction file system} structure of all index related folders, lucene {@link
 * Directory directories} and {@link FailureStorage failure storage}.
 */
public class PartitionedIndexStorage {
  ...
  /**
   * For the given {@link Path folder} removes all nested folders from both {@link
   * FileSystemAbstraction file system} and {@link Directory lucene directories}.
   *
  ...
  /**
   * Opens all {@link Directory lucene directories} contained in the {@link #getIndexFolder() index
   * folder}.
   *
   * @return the map from file system {@link Path directory} to the corresponding {@link Directory
   *     lucene directory}.
   * @throws IOException if opening of some lucene directory (via {@link
   *     DirectoryFactory#open(Path)}) fails.
   */
   ...
   *
   * @return the list of index partition folders or {@link Collections#emptyList() empty list} if
   *     index folder is empty.
   */
  ...
  /**
   * Removes content of the lucene directory denoted by the given {@link Path file}. This might seem
   * unnecessary since we cleanup the folder using {@link FileSystemAbstraction file system} but in
   * fact for testing we often use in-memory directories whose content can't be removed via the file
   * system.
   *
   * <p>Uses {@link FileUtils#windowsSafeIOOperation(FileUtils.Operation)} underneath.
   *
