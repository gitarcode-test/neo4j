...
@Command(
    ...
    footer = {
      ...
      "  HEAP_SIZE     Set JVM maximum heap size during command execution. Takes a number and a"
          + " unit, for example 512m.",
...
public class AdminTool {
    ...
    return getCommandLine(
        ...
        new Strategy() {
          ...
          public void registerCommandsFromGroup(
            ...
            CommandLine groupCommandLine =
                new CommandLine(commandSpec, new ContextInjectingFactory(ctx))
                    .addSubcommand(null, HelpCommand.class, "-h", "--help");
            registerGroupCommands(
                commandProviders, commandGroup, ctx, groupCommandLine, type -> true);
  ...
  protected static CommandLine getCommandLine(ExecutionContext ctx, Strategy strategy) {
    CommandLine cmd =
        new CommandLine(strategy.createRootCommand(), new ContextInjectingFactory(ctx));
  ...
  private static void registerGroupCommands(
    ...
    List<Object> subcommands =
        filterCommandProviders(commandProviders, commandGroup).stream()
  ...
  @Command(
      name = "neo4j",
      description =
          "A partial alias for 'neo4j-admin server'. Commands for working with DBMS process "
              + "from 'neo4j-admin server' category can be invoked using this command.")
  public static class Neo4jAlias extends AdminTool {

    private static final Set<CommandType> SUPPORTED_COMMANDS =
        Set.of(
