...
abstract class PropertyIndexSeekPartitionedScanTestSuite<CURSOR extends Cursor>
  ...
  private static final Pair<Integer, Integer> RANGE =
      Pair.of(Integer.MIN_VALUE / 2, Integer.MAX_VALUE / 2);
        ...
        final var validQueries =
            Stream.concat(
                        ...
                        .flatMap(
                            propKeyId ->
                                Arrays.stream(ValueType.values())
                                    .map(
                                        type ->
                                            new PropertyRecord(propKeyId, type.toValue(0), type)))
                        .flatMap(
                            prop ->
                                queries(prop)
                                    .map(
                                        query ->
                                            new PropertyKeySeekQuery(
                    ...
                    queries(
                            Arrays.stream(propKeyIds)
                                .mapToObj(
                                    propKeyId -> createRandomPropertyRecord(random, propKeyId, 0))
  ...
  private static Stream<PropertyIndexQuery[]> queries(PropertyRecord... props) {
    ...
    for (final var singleQueries : allSingleQueries) {
      compositeQueries =
          compositeQueries.flatMap(
              prev ->
                  singleQueries.stream()
                      .map(
                          extra -> {
    ...
    return compositeQueries.map(
        compositeQuery -> compositeQuery.toArray(PropertyIndexQuery[]::new));
  }

  /**
   * Used to keep track of what entity ids we expect to find from different queries. In "tracking"
   * we keep track of all queries and all nodes. In "included" we keep track of the queries we want
   * to test. There will be a lot of different exact queries so we randomly select a few of them to
   * test.
   */
  protected static final class TrackEntityIdsMatchingQuery {
    private final EntityIdsMatchingQuery<PropertyKeySeekQuery> tracking =
        new EntityIdsMatchingQuery<>();
    private final EntityIdsMatchingQuery<PropertyKeySeekQuery> included =
        new EntityIdsMatchingQuery<>();
    ...
    void generateAndTrack(
      ...
      validQueries.get(true).stream()
          ...
          .filter(
              query ->
                  Arrays.stream(query.get())
                          .noneMatch(PropertyIndexQuery.ExactPredicate.class::isInstance)
                      || includeExactQueries)
    ...
    public String toString() {
      return String.format(
          ...
          Arrays.stream(queries)
              .map(PropertyIndexQuery::toString)
