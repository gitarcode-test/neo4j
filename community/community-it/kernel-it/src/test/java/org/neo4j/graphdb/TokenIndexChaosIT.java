...
/**
 * Tests functionality around missing or corrupted token indexes, and that the database should
 * repair (i.e. rebuild) that automatically and just work.
 */
@DbmsExtension
          ...
          .map(
              idx -> {
                IndexDirectoryStructure indexDirectoryStructure =
                    directoriesByProvider(db.databaseLayout().databaseDirectory())
