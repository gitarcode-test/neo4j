...
public class TransactionTerminationIT {
  ...
  @Inject private Neo4jWithSocket server;
  ...
  @Timeout(15)
  @ProtocolTest
  void killTxViaReset(BoltWire wire, @Authenticated TransportConnection connection)
      throws Exception {
  ...
  @Timeout(15)
  @ProtocolTest
  void killTxThenTryToUseItTest(BoltWire wire, @Authenticated TransportConnection connection)
      throws Exception {
    connection
        .send(wire.begin())
    ...
    try (var tx = server.graphDatabaseService().beginTx()) {
      ...
      var terminationResult =
          tx.execute(String.format("TERMINATE TRANSACTION \"%s\"", transactionId));
    ...
    assertThat(connection)
        .receivesFailure(
            ...
            "The transaction has been terminated. Retry your operation in a new transaction, and"
                + " you should see a successful result. Explicitly terminated by the user. ");
  }

  @Timeout(20)
  @ProtocolTest
  void killedTxShouldNotDestroyConnection(
      BoltWire wire, @Authenticated TransportConnection connection) throws Exception {
    connection
        .send(wire.begin())
        .send(wire.run("UNWIND range(1, 200) AS i RETURN i"))
        .send(wire.pull());
    ...
    try (var tx = server.graphDatabaseService().beginTx()) {
      var result = tx.execute("SHOW TRANSACTIONS");
      var unwindTransaction =
          result.stream().toList().stream()
              .filter(x -> !x.get("connectionId").equals("") && !x.get("clientAddress").equals(""));
