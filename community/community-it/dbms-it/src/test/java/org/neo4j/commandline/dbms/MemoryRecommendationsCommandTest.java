...
class MemoryRecommendationsCommandTest {
  ...
  void printUsageHelp() {
    ...
    final var command =
        new MemoryRecommendationsCommand(new ExecutionContext(Path.of("."), Path.of(".")));
  ...
  void mustRecommendPageCacheMemoryWithOffHeapTxState() {
    assertThat(recommendPageCacheMemory(mebiBytes(100), mebiBytes(130)))
        .isBetween(mebiBytes(7), mebiBytes(12));
    assertThat(recommendPageCacheMemory(gibiBytes(1), mebiBytes(260)))
        .isBetween(mebiBytes(8), mebiBytes(50));
    assertThat(recommendPageCacheMemory(gibiBytes(3), mebiBytes(368)))
        .isBetween(mebiBytes(100), mebiBytes(256));
    assertThat(recommendPageCacheMemory(gibiBytes(6), mebiBytes(780)))
        .isBetween(mebiBytes(100), mebiBytes(256));
    assertThat(recommendPageCacheMemory(gibiBytes(192), gibiBytes(10)))
        .isBetween(gibiBytes(75), gibiBytes(202));
    assertThat(recommendPageCacheMemory(gibiBytes(1920), gibiBytes(10)))
        .isBetween(gibiBytes(978), gibiBytes(1900));

    // Also never recommend more than 16 TiB of page cache memory, regardless of how much is
    // available.
    assertThat(recommendPageCacheMemory(exbiBytes(1), gibiBytes(100)))
        .isLessThanOrEqualTo(tebiBytes(16));
  }
  ...
  void mustRecommendPageCacheMemoryWithOnHeapTxState() {
    ...
    assertThat(recommendPageCacheMemory(gibiBytes(6), 0))
        .isBetween(mebiBytes(728), mebiBytes(1056));
    assertThat(recommendPageCacheMemory(gibiBytes(192), 0))
        .isBetween(gibiBytes(75), gibiBytes(202));
    assertThat(recommendPageCacheMemory(gibiBytes(1920), 0))
        .isBetween(gibiBytes(978), gibiBytes(1900));
  ...
  void mustPrintRecommendationsAsConfigReadableOutput() throws Exception {
    ...
    MemoryRecommendationsCommand command =
        new MemoryRecommendationsCommand(
        ...
        .println(
            contains(
                "Total size of lucene indexes in all databases: "
                    + bytesToString(expectedLuceneIndexesSize)));
    verify(output)
        .println(
            contains(
  ...
  private long[] calculatePageCacheFileSize(DatabaseLayout databaseLayout) throws IOException {
    ...
    for (final var path :
        StorageEngineFactory.selectStorageEngine(fs, databaseLayout)
            .orElseThrow(
                () ->
                    new IOException(
                        "No storage engine factory matched for layout: " + databaseLayout))
  ...
  private static void createDatabaseWithIndexes(Path homeDirectory, String databaseName) {
    ...
    var dbms =
        new TestDatabaseManagementServiceBuilder(homeDirectory)
