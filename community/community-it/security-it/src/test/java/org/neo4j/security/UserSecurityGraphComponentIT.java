...
class UserSecurityGraphComponentIT {
  ...
  private static final SystemGraphComponent.Name testComponent =
      new SystemGraphComponent.Name("test-component");

  @BeforeAll
  static void setup() {
    Config cfg =
        Config.newBuilder().set(auth_enabled, TRUE).set(automatic_upgrade_enabled, FALSE).build();
    ...
    internalSystemGraphComponentsBuilder.register(
        new StubComponent(SECURITY_USER_COMPONENT.name()));
    ...
    userSecurityGraphComponent =
        new UserSecurityGraphComponent(
  ...
  void shouldInitializeDefaultVersion() throws Exception {
    ...
    HashMap<SystemGraphComponent.Name, SystemGraphComponent.Status> componentStatuses =
        new HashMap<>();
    SystemGraphComponent.Name overallStatus =
        new SystemGraphComponent.Name(
            "overall-status"); // aggregated status, rather than a real component
    inTx(
        tx -> {
  ...
  void shouldInitializeAndUpgradeSystemGraph(
      UserSecurityGraphComponentVersion version, SystemGraphComponent.Status initialStatus)
      throws Exception {
    initUserSecurityComponent(version);
  ...
  void shouldAddUserIdsOnUpgradeFromOlderSystemDb(UserSecurityGraphComponentVersion version)
      throws Exception {
    // Given
    initUserSecurityComponent(version);
    var systemGraphComponents =
  ...
  void shouldAddConstraintForUserIdsOnUpgradeFromOlderSystemDb(
      UserSecurityGraphComponentVersion version) throws Exception {
    // Given
    initUserSecurityComponent(version);
    var systemGraphComponents =
        systemGraphComponentsPlus(internalSystemGraphComponents, userSecurityGraphComponent);
    ...
    try (Transaction tx =
        system.beginTransaction(KernelTransaction.Type.EXPLICIT, LoginContext.AUTH_DISABLED)) {
  ...
  private static void assertCanUpgradeThisVersionAndThenUpgradeIt(
      SystemGraphComponent.Status initialState) throws Exception {
  ...
  private static void assertStatus(
    ...
    inTx(
        tx -> {
          systemGraphComponents.forEach(
              component -> statuses.put(component.componentName(), component.detect(tx)));
  ...
  private static void initUserSecurityComponent(UserSecurityGraphComponentVersion version)
      throws Exception {
    KnownCommunitySecurityComponentVersion builder =
        userSecurityGraphComponent.findSecurityGraphComponentVersion(version);
