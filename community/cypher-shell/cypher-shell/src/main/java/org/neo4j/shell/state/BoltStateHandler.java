...
public class BoltStateHandler implements TransactionHandler, Connector, DatabaseManager {
  ...
  private static final TransactionConfig USER_DIRECT_TX_CONF =
      txConfig(TransactionType.USER_DIRECT);
  ...
  @VisibleForTesting
  BoltStateHandler(
      TriFunction<URI, AuthToken, Config, Driver> driverProvider, boolean isInteractive) {
  ...
  /**
   * Handle an exception while getting or consuming the result. If not in TX, return the given
   * exception. If in a TX, terminate the TX and return a more verbose error message.
   *
  ...
  public Neo4jException handleException(Neo4jException e) {
    if (isTransactionOpen()) {
      ...
      return new ErrorWhileInTransactionException(
          "An error occurred while in an open transaction. The transaction will be rolled back and"
              + " terminated. Error: "
              + e.getMessage(),
          e);
    } else {
  ...
  public void reconnect() throws CommandException {
    ...
    if (isTransactionOpen()) {
      throw new CommandException(
          "There is an open transaction. You need to close it before you can reconnect.");
    }
  ...
  public void connect(ConnectionConfig incomingConfig) throws CommandException {
    ...
    final AuthToken authToken =
        AuthTokens.basic(connectionConfig.username(), connectionConfig.password());
    try {
      String previousDatabaseName = activeDatabaseNameAsSetByUser;
      try {
  ...
  private void reconnectAndPing(String databaseToConnectTo, String previousDatabase)
  ...
  private LicenseDetails getTrialStatus() {
    try {
      final var record =
          session.run("CALL dbms.licenseAgreementDetails()", SYSTEM_TX_CONF).single();
  ...
  public String getServerVersion() {
    try {
      return runCypher(
              "CALL dbms.components() YIELD versions", Collections.emptyMap(), SYSTEM_TX_CONF)
  ...
  @Override
  public Optional<BoltResult> runUserCypher(String cypher, Map<String, Value> queryParams)
      throws CommandException {
    return runCypher(cypher, queryParams, USER_DIRECT_TX_CONF);
  }

  @Override
  public Optional<BoltResult> runCypher(
      String cypher, Map<String, Value> queryParams, TransactionType type) throws CommandException {
    return runCypher(cypher, queryParams, txConfig(type));
  }

  private Optional<BoltResult> runCypher(
      String cypher, Map<String, Value> queryParams, TransactionConfig config)
      throws CommandException {
    if (!isConnected()) {
      throw new CommandException("Not connected to Neo4j");
    }
    if (isTransactionOpen()) {
      ...
      try {
        // Note that CALL IN TRANSACTIONS can't execute in an explicit transaction, so if the user
        // has not typed
        // BEGIN, then
  ...
  public void changePassword(ConnectionConfig connectionConfig, String newPassword) {
    ...
    try {
      ...
      try {
        ...
        Result run =
            session.run(new Query(command, parameters), txConfig(TransactionType.USER_ACTION));
        run.consume();
   ...
   */
  private Optional<BoltResult> getBoltResult(
      String cypher, Map<String, Value> queryParams, TransactionConfig config)
  ...
  /**
   * Disconnect from Neo4j, clearing up any session resources, but don't give any output. Intended
   * only to be used if connect fails.
   */
