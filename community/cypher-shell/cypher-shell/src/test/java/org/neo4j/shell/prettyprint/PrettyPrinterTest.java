...
class PrettyPrinterTest {
  private final PrettyPrinter plainPrinter =
      new PrettyPrinter(new PrettyConfig(Format.PLAIN, false, 100, false));
  ...
  void prettyPrintExplainInformation() {
    ...
    when(resultSummary.queryType()).thenReturn(QueryType.READ_ONLY);
    Map<String, Value> argumentMap =
        Values.parameters("Version", "3.1", "Planner", "COST", "Runtime", "INTERPRETED")
            .asMap(v -> v);
    when(plan.arguments()).thenReturn(argumentMap);
  ...
  void prettyPrintSingleNodePath() {
    ...
    // then
    assertThat(actual)
        .isEqualTo("path" + NEWLINE + "(:start)-[:RELATIONSHIP_TYPE]->(:end)" + NEWLINE);
  }
