...
class JavaCcCypherLanguageServiceTest {
  ...
  void tokenizeSanity() {
    ...
    assertTokens(
        "MATCH (myNode:MyLabel { mapKey: 'value' }) WHERE myNode.p = 1 RETURN count(myNode)",
        tokens);
  ...
  void querySuggestionsSanity() {
    ...
    assertSuggestions(
        "", "CREATE", "MATCH", "DROP", "UNWIND", "RETURN", "WITH", "LOAD", "ALTER", "RENAME",
        "SHOW", "START", "STOP");
    assertSuggestions(
        "MATCH (n) ", "WHERE", "MATCH", "RETURN", "UNWIND", "MERGE", "DELETE", "WITH", "UNION");
  ...
  private Condition<CypherLanguageService.Token> token(String image, int begin, int end) {
    return new Condition<>(
        token ->
            token.image().equals(image) && token.beginOffset() == begin && token.endOffset() == end,
        "Token with image=" + image + ", beginOffset=" + begin + ", endOffset=" + end);
  }
