...
/**
 * Given a procedure that outputs <tt>Stream<MyOut></tt> we look for all public fields of
 * <tt>MyOut</tt> to produce a signature.
 */
class ProcedureOutputSignatureCompiler {
  ...
  List<FieldSignature> fieldSignatures(Class<?> userClass) throws ProcedureException {
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      if (!userClass.isRecord() && !isPublic(field.getModifiers())) {
        throw new ProcedureException(
            Status.Procedure.TypeError,
            "Field `%s` in record `%s` cannot be accessed. Please ensure the field is marked as"
                + " `public`.",
            field.getName(),
  ...
  private ProcedureException invalidReturnType(Class<?> userClass) {
    return new ProcedureException(
        Status.Procedure.TypeError,
        "Procedures must return a Stream of records, where a record is a concrete class%nthat you"
            + " define, with public non-final fields defining the fields in the record.%nIf you''d"
            + " like your procedure to return `%s`, you could define a record class like:%npublic"
            + " class Output '{'%n    public %s out;%n'}'%n%nAnd then define your procedure as"
            + " returning `Stream<Output>`.",
        userClass.getSimpleName(),
        userClass.getSimpleName());
  }
