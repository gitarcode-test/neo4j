...
public class TypeCheckers {
  ...
  private static final Function<String, DefaultParameterValue> PARSE_STRING =
      DefaultParameterValue::ntString;
  private static final Function<String, DefaultParameterValue> PARSE_INTEGER =
      s -> ntInteger(parseLong(s));
  private static final Function<String, DefaultParameterValue> PARSE_FLOAT =
      s -> ntFloat(parseDouble(s));
  ...
  private static final Function<String, DefaultParameterValue> PARSE_BOOLEAN =
      s -> ntBoolean(parseBooleanOrFail(s));
  ...
  private static final CompositeConverter PARSE_ANY =
  ...
  private static final DefaultValueConverter TO_STRING =
      new DefaultValueConverter(NTString, PARSE_STRING);
  private static final DefaultValueConverter TO_INTEGER =
      new DefaultValueConverter(NTInteger, PARSE_INTEGER);
  private static final DefaultValueConverter TO_FLOAT =
      new DefaultValueConverter(NTFloat, PARSE_FLOAT);
  private static final DefaultValueConverter TO_NUMBER =
      new DefaultValueConverter(NTNumber, PARSE_NUMBER);
  private static final DefaultValueConverter TO_BOOLEAN =
      new DefaultValueConverter(NTBoolean, PARSE_BOOLEAN);
  ...
  private final DefaultValueConverter TO_BYTE_ARRAY =
      new DefaultValueConverter(NTByteArray, PARSE_BYTE_ARRAY);
  ...
  /**
   * We don't have Node, Relationship, Property available down here - and don't strictly want to, we
   * want the procedures to be independent of which Graph API is being used (and we don't want them
   * to get tangled up with kernel code). So, we only register the "core" type system here, scalars
   * and collection types. Node, Relationship, Path and any other future graph types should be
   * registered from the outside in the same place APIs to work with those types is registered.
   */
  ...
  private static DefaultValueConverter toList(DefaultValueConverter inner, Type type) {
    return new DefaultValueConverter(
        NTList(inner.type()), new ListConverter(type, inner.type(), EVALUATOR));
  }
  ...
  public static final class DefaultValueConverter extends TypeChecker {
    ...
    public Optional<DefaultParameterValue> defaultValue(String defaultValue)
