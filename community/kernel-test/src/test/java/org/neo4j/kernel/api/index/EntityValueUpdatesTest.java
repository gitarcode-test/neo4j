...
/**
 * NOTE: Test methods are parametrised to be executed for both nodes an relationship where it makes
 * sense. In other words, if a test is executed just for a node, it means the operation does not
 * make semantically sense for a relationship. The Set of operations that can be executed on a node
 * is much larger than relationship equivalent. The biggest difference is that relationships can
 * have exactly one token and adding or removing it equals adding or removing the relationships
 * itself, so some situation tested for nodes cannot happen for relationships. A couple of examples
 * of such operations for illustration:
 *
 * <ul>
 *   <li>Adding a token to an entity with existing properties
 *   <li>Removing a token, but leaving the properties unchanged
 *   <li>Any operation with more than one token
 *   <li>...
 * </ul>
 */
class EntityValueUpdatesTest {
  ...
  private static final StorageProperty PROPERTY_1 =
      new PropertyKeyValue(PROPERTY_KEY_ID_1, Values.of("Neo"));
  private static final StorageProperty PROPERTY_2 =
      new PropertyKeyValue(PROPERTY_KEY_ID_2, Values.of(100L));
  private static final StorageProperty PROPERTY_3 =
      new PropertyKeyValue(
          PROPERTY_KEY_ID_3, Values.pointValue(CoordinateReferenceSystem.WGS_84, 12.3, 45.6));
  private static final Value[] VALUES_123 =
      new Value[] {PROPERTY_1.value(), PROPERTY_2.value(), PROPERTY_3.value()};
  ...
  void shouldNotGenerateUpdateForMultipleExistingPropertiesAndTokens(Entity entity) {
    // When
    EntityUpdates updates =
        EntityUpdates.forEntity(ENTITY_ID, false)
            .withTokens(TOKEN)
    ...
    assertThat(
        ...
        .contains(
            IndexEntryUpdate.add(
                ENTITY_ID, entity.nonSchemaIndex(), PROPERTY_1.value(), null, null));
  }
    ...
    assertThat(
        ...
        .contains(
            IndexEntryUpdate.change(
                ...
                entity.nonSchemaIndex(),
                VALUES_123,
                new Value[] {newValue1, newValue2, newValue3}));
  }
  ...
  private static StorageReader propertyLoader(StorageProperty... properties) {
    ...
    for (StorageProperty property : properties) {
      stub.propertyKeyTokenHolder()
          .addToken(
              new NamedToken(String.valueOf(property.propertyKeyId()), property.propertyKeyId()));
    }
  ...
  private static StorageReader assertNoLoading() {
    ...
    IllegalStateException exception =
        new IllegalStateException("Should never attempt to load properties!");
    ...
    RELATIONSHIP {
      ...
      private final SchemaDescriptorSupplier index123 =
          () ->
              SchemaDescriptors.forLabel(
                  TOKEN_ID_1, PROPERTY_KEY_ID_1, PROPERTY_KEY_ID_2, PROPERTY_KEY_ID_3);
      private final List<SchemaDescriptorSupplier> indexes =
          Arrays.asList(index1, index2, index3, index123);
