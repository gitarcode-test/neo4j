...
class TransactionLogFileInformationTest {
  ...
  @Test
  void shouldReadAndCacheFirstCommittedTransactionIdForAGivenVersionWhenNotCached()
      throws Exception {
    TransactionLogFileInformation info =
        new TransactionLogFileInformation(logFiles, logHeaderCache, context);
    long expected = 5;
  ...
  void extractLogFileTimeFromChunkStartEntry() throws IOException {
    ...
    when(logEntryReader.readLogEntry(readableLogChannel))
        .thenReturn(
            new LogEntryChunkStart(
                LatestVersions.LATEST_KERNEL_VERSION, 42, 1, LogPosition.UNSPECIFIED));
    var fileInfo =
        new TransactionLogFileInformation(logFiles, logHeaderCache, context, () -> logEntryReader);
